    ...
    for line in f:
        ...
        if line == '\n':
            ...
            if simplexes and iteration >= draw_from_iteration:
                show_convex_hull(simplexes, selected, wanted, title=title)
            simplexes = []
            selected = []
            continue
...
def show_convex_hull(simplexes, selected=[], wanted=[], show=True, title=''):
    '''Shows partitioning and convex hull '''
    from matplotlib import pyplot as plt
    ## Draw two plots
