...
void Simplex::update_min_lb_values(vector<Simplex*> simpls, Function* func) {
    for (int sid=0; sid < simpls.size(); sid++) {
        if (simpls[sid]->_should_lb_mins_be_updated or Simplex::glob_L_was_updated) {
            //// Find local Lipschitz constant.
            double local_L = 0;
            for (int v=0; v < simpls[sid]->_verts.size(); v++) {
                for (int k=0; k < simpls[sid]->_verts[v]->_simplices.size(); k++) {
                    if (simpls[sid]->_verts[v]->_simplices[k]->_min_L > local_L) {
                        local_L = simpls[sid]->_verts[v]->_simplices[k]->_min_L;
                    };
                };
            };
