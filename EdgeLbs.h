...
public:
    EdgeLbs(vector<Point*> verts, double L) {
        ...
        for (int a=0; a <= _D; a++) {
            for (int b=a; b <= _D; b++) {
                ...
                if (a != b) {
                    ...
                    ca = 0.5 + (_verts[b]->_value - _verts[a]->_value) / (2 * _L * dist_ab);
                    ...
                    double expected_lb_value = _verts[b]->_value - _L * dist_ab * ca;
